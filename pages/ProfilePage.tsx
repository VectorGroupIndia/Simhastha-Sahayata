...
const PilgrimProfileBody: React.FC<{ user: User; onUpdateUser: (data: Partial<User>) => void; onSelectSosAlert: (alert: SosAlert) => void; }> = ({ user, onUpdateUser, onSelectSosAlert }) => {
    ...
    const handleSettingsUpdate = (update: Partial<User['settings']>) => {
        ...
        // Simulate persistence by updating the main DEMO_USERS array
        const userIndex = DEMO_USERS.findIndex(u => u.id === user.id);
        if (userIndex !== -1 && DEMO_USERS[userIndex].settings) {
            DEMO_USERS[userIndex].settings = { ...DEMO_USERS[userIndex].settings, ...update };
        }
    };
        ...
        <div className="mt-6 space-y-6">
            {activeTab === 'safety' && (
             ...
             {activeTab === 'settings' && (
                <div className="animate-fade-in space-y-6">
                    <Card>
                        <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white">Appearance</h2>
                        <div className="flex justify-between items-center">
                            <div>
                                <p className="font-medium text-gray-800 dark:text-gray-200">{t.theme}</p>
                                <p className="text-sm text-gray-500 dark:text-gray-400">{t.themeDesc}</p>
