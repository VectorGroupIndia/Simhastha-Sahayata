...
const ReportLostFoundPage: React.FC = () => {
  ...
  const handleReview = (e: React.FormEvent) => {
    ...
    const reportData: LostFoundReport = {
        ...
        ...(category === 'Item' && { itemName, itemBrand, itemColor, identifyingMarks, subCategory, itemMaterial, itemSize }),
    };
  ...
  const handleAutofill = async () => {
      ...
      try {
          ...
          if (autofilledData.subCategory) setSubCategory(autofilledData.subCategory);
          ...
          if (autofilledData.itemMaterial) setItemMaterial(autofilledData.itemMaterial);
          if (autofilledData.itemSize) setItemSize(autofilledData.itemSize);
      } catch (error) {
  ...
  const handleAnalyzeImage = async () => {
    if (!imageBase64) {
        addToast(translations.report.uploadFirst, 'error');
        return;
    };
    setIsAnalyzing(true);
    try {
        ...
        if (analysisData.subCategory) setSubCategory(analysisData.subCategory);
        ...
        if (analysisData.itemMaterial) setItemMaterial(analysisData.itemMaterial);
        if (analysisData.itemSize) setItemSize(analysisData.itemSize);
        ...
        return (
         <>
          <form onSubmit={handleReview} className="space-y-6">
            ...
            {/* AI Autofill Section */}
            <div>
              <label htmlFor="ai-prompt" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{translations.report.aiPromptLabel}</label>
              <div className="flex gap-2">
                <textarea id="ai-prompt" value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} rows={2} placeholder={translations.report.aiPromptPlaceholder} className="flex-grow bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-orange-500 focus:ring-orange-500" />
                <Button type="button" onClick={handleAutofill} disabled={isAutofilling || !aiPrompt.trim()} className="h-full"><SparklesIcon /><span className="hidden sm:inline ml-1">{translations.report.aiFillButton}</span></Button>
              </div>
            </div>
                ...
                {imageBase64 ? (
                    ...
                    <div onDrop={handleDrop} onDragOver={handleDragOver} onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} className={`mt-1 p-4 border-2 border-dashed rounded-md transition-colors ${isDraggingOver ? 'border-orange-500 bg-orange-50 dark:bg-orange-900/20' : 'border-gray-300 dark:border-gray-600'}`}>
                        <div className="space-y-1 text-center">
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">{translations.report.uploadHint}</p>
                 ...
                 <Button type="button" onClick={handleAnalyzeImage} disabled={isAnalyzing || !imageBase64} variant="secondary" className="w-full mt-2">
                    {isAnalyzing ? <><Spinner size="sm" className="mr-2" />{translations.report.aiAnalyzing}</> : <><CameraIcon />{translations.report.analyze}</>}
                 </Button>
            </div>
            
            <Button type="submit" className="w-full" disabled={!category}>{translations.report.reviewButton}</Button>
          </form>
          {imageBase64 && (
            <ImageZoomModal 
